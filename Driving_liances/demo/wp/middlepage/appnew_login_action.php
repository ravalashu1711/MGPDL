<?php	
	...
	if(isset($_POST['submit']))
	{
		...
		$sql = "SELECT * FROM `payment` WHERE `pay_id`='$pay_id' AND `u_id`='$u_id' AND `status`='Active' AND `pay_type`='New License'";
		$query = $con->query($sql);
		...
		if($row<=0)
		{
			...
			header('location:index.php?page=appnew_login&msg='.$msg);
		}
