<?php
	    ...
	    if(isset($_POST['submit']))
		{
			...
			$sql_new_licence = "SELECT * FROM `renew_license` ORDER BY `rl_id` DESC";
			$query_new_licence = $con->query($sql_new_licence);
			$data_new_licence = $query_new_licence->fetch(PDO::FETCH_ASSOC);
			$last_id = $data_new_licence['rl_id'];
			if($query <=1)
			{

				move_uploaded_file($tmp_name,"../upload/".$document);
				
				header('location:../index.php?page=license_view&msg=Successfull&rl_id='.$last_id);
			}
			else
			{
